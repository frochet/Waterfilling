...
int pack_chn_led_escrow(chn_led_escrow_t token, byte(*proto_id)[DIGEST_LEN], byte** str_out){
    return pack_token(MT_NTYPE_CHN_LED_ESCROW, &token, sizeof(token), proto_id, str_out);
}
...
int unpack_chn_led_escrow(byte* str, int size, chn_led_escrow_t* tkn_out, byte(*id_out)[DIGEST_LEN]){
  if(size != sizeof(mt_ntype_t) + sizeof(*tkn_out) + DIGEST_LEN)
    return MT_ERROR;
  return unpack_token(MT_NTYPE_CHN_LED_ESCROW, str, sizeof(*tkn_out), tkn_out, id_out);
}
