...
///int transfer(mt_ledger_t* ledger, int* bal_from, int* bal_to, int val_from, int val_to, int val_auth);
...
////**
 ...
 ///int mt_ledger_init(mt_ledger_t* ledger, mt_send_cells send_cells, mt_close_conn close_conn,
    ...
    ///    ledger->send_cells = send_cells;
	 ...
	 ///	return MT_ERROR;
     ///
	 ///    g_tree_insert(ledger->mac_accounts, ledger->auth_addr, calloc(1, sizeof(mac_led_data_t)));
     ///    return MT_SUCCESS;
     ///}
///
////**
...
///
  ...
  ///    mac_led_data_t* data = g_tree_lookup(ledger->mac_accounts, ledger->auth_addr);
...
///int handle_chn_int_cashout(mt_ledger_t* ledger, chn_int_cashout_t* token, byte (*addr)[MT_SZ_ADDR]){
    ...
    ///    chn_led_data_t* data_chn = g_tree_lookup(ledger->chn_accounts, token->chn);
     ...
     ///
	 ///    // check that the transfer goes through
	 ///    int val_auth = ledger->fee + token->val_to * ledger->tax;
...
///    *bal_to += val_to;
///
///    mac_led_data_t* auth_data = g_tree_lookup(ledger->mac_accounts, ledger->auth_addr);
