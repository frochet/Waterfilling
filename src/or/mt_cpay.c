...
typedef struct {
  ...
  digestmap_t* chns_transition;   // proto_id -> channsl
...
static int handle_nan_rel_reqclose2(mt_desc_t* desc, nan_rel_reqclose2_t* token, byte (*proto_id)[DIGEST_LEN]);
...
int mt_cpay_recv(mt_desc_t* desc, mt_ntype_t type, byte* msg, int size){
  ...
  switch(type){
    ...
    case MT_NTYPE_NAN_REL_REQCLOSE2:;
      ...
      if(unpack_nan_rel_reqclose2(msg, size, &nan_rel_reqclose2_tkn, &proto_id) != MT_SUCCESS)
	return MT_ERROR;
      result = handle_nan_rel_reqclose2(desc, &nan_rel_reqclose2_tkn,  &proto_id);
      break;
...
static int init_nan_cli_reqclose1(mt_event_notify_t notify, mt_channel_t* chn,  mt_desc_t* desc){
  ...
  // send message
  byte* msg;
  int msg_size = pack_nan_cli_reqclose1(&token, (byte (*)[DIGEST_LEN])&proto_id, &msg);
...
static int handle_any_led_confirm(mt_desc_t* desc, any_led_confirm_t* token, byte (*proto_id)[DIGEST_LEN]){
  ...
  digestmap_remove(client.chns_transition, (char*)*proto_id);
...
static int handle_nan_rel_estab6(mt_desc_t* desc, nan_rel_estab6_t* token, byte (*proto_id)[DIGEST_LEN]){
  ...
  digestmap_remove(client.chns_transition, (char*)*proto_id);
  digestmap_set(client.nans_estab, (char*)*proto_id, chn);
...
static int handle_nan_rel_reqclose2(mt_desc_t* desc, nan_rel_reqclose2_t* token, byte (*proto_id)[DIGEST_LEN]){
  ...
  digestmap_remove(client.chns_transition, (char*)*proto_id);
  digestmap_set(client.nans_reqclosed, (char*)*proto_id, chn);
